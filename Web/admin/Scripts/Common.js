...
function AjaxFormValidator(DecUCodeID, UCodeID, RecUCodeID, ConUcodeID,BtnID) {
    ...
    $("#" + DecUCodeID).formValidator({ onShow: "请输入服务中心编号", onFocus: "服务中心编号不能为空", onCorrect: "输入正确" }).inputValidator({ min: 6, max: 18, onError: "编号输入错误" }).ajaxValidator({ data: { CheckValue: $("#" + DecUCodeID).val(), flag: "1" }, dataType: "html", async: true, url: "FormValidatorHandler.ashx", success: function(data) { if (data.indexOf("服务中心可以使用!") > 0) return true; return data; }, buttons: $("#" + BtnID), error: function(jqXHR, textStatus, errorThrown) { alert("服务器没有返回数据，可能服务器忙，请重试" + errorThrown); }, onError: "服务中心：不是服务中心或者不存在", onWait: "正在对服务中心编号进行合法性校验，请稍候..." });
...
function FormValidator(DecUCodeID, UCodeID, RecUCodeID, ConUcodeID, UPassID, UPassAgainID, UPassID2, UPassAgainID2, UPassID3, UPassAgainID3, BankBranchID, BankCodeID, BankUNameID, UNameID, UCodeNumID, UPhoneID, UAddressID) {
    $.formValidator.initConfig({ formID: "form1", theme: 'ArrowSolidBox', mode: 'AutoTip', onError: function(msg) { alert(msg) }, inIframe: true });
    $("#" + DecUCodeID).formValidator({ onShow: "请输入服务中心编号", onFocus: "服务中心编号不能为空", onCorrect: "输入正确" }).inputValidator({ min: 6, max: 18, onError: "编号输入错误" });
