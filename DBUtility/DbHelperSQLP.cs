...
namespace DataAccess
{
    ...
    public class DbHelperSQLP
    {
        ...
        /// 自动分配参数
        /// </summary>
        protected static void AssignParams(object[] Params, IDbCommand idb)
        {
            int i = 0;
            idb.CommandTimeout = 36000;
            IEnumerator ienum = Params.GetEnumerator();
            if (idb.CommandType == CommandType.StoredProcedure)
            {
                SqlCommandBuilder.DeriveParameters((SqlCommand)idb); //自动指定参数
                while (ienum.MoveNext())
                {
                    ((SqlCommand)idb).Parameters[++i].Value = ienum.Current;
                }
            }
            else
            {
